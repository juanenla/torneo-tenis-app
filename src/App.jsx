...
// --- NUEVO: Importamos la librería para generar imágenes ---
...
export default function App() {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            ...
            else setAllMatches(allMatchesData || []);
        };
        fetchInitialData();
    }, []);
    ...
    useEffect(() => {
        if (players.length > 0) {
            setLeaderboardData(calculateLeaderboard(players, allMatches));
        }
    }, [players, allMatches]);
    ...
    const handleAddMatch = async (newMatchData) => {
        try {
            ...
            if (data && data.length > 0) {
                setAllMatches(currentMatches => [...currentMatches, data[0]]);
            }
            alert("Resultado cargado y tabla actualizada. Pendiente de aprobación final.");
        } catch (error) {
    ...
    const handleShareAsImage = async (elementId, fileName) => {
        ...
        try {
            ...
            shareButtons.forEach(button => button.style.visibility = 'hidden');
            ...
            shareButtons.forEach(button => button.style.visibility = 'visible');
            ...
            element.querySelectorAll('.share-button').forEach(button => button.style.visibility = 'visible');
        } finally {
            setIsSharing(false);
        }
    };
    ...
    return (
        <div className={`bg-gray-900 text-white min-h-screen p-4 sm:p-6 md:p-8 ${isSharing ? 'cursor-wait' : ''}`}>
            <div className="max-w-7xl mx-auto">
                ...
                {/* --- SECCIÓN RANKING CON TÍTULO MODIFICADO PARA LA IMAGEN --- */}
                <div id="leaderboard-section" className="bg-gray-800 rounded-lg shadow-lg p-4">
                    <div className="flex justify-between items-center mb-4">
                        <div>
                            <h2 className="text-2xl font-bold text-cyan-400">Tabla de Posiciones</h2>
                            <p className="text-sm text-gray-400">Liga Cubanitos - Categoría B</p>
                        </div>
                        <button onClick={() => handleShareAsImage('leaderboard-section', 'ranking-torneo')} disabled={isSharing} className="share-button flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-cyan-300 bg-cyan-500/10 border border-cyan-500/30 rounded-md hover:bg-cyan-500/20 disabled:opacity-50 disabled:cursor-wait">
                ...
                {/* --- SECCIÓN "ÚLTIMOS PARTIDOS" CON TÍTULO CENTRADO --- */}
                {pendingMatches.length > 0 && (
                    <div id="last-matches-section" className="mt-8 bg-gray-800 rounded-lg shadow-lg p-4">
                        <div className="relative text-center mb-4">
                            <h3 className="text-2xl font-bold text-yellow-400">Últimos Partidos Jugados</h3>
                             <button onClick={() => handleShareAsImage('last-matches-section', 'ultimos-partidos')} disabled={isSharing} className="share-button absolute top-1/2 right-0 -translate-y-1/2 flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-yellow-300 bg-yellow-500/10 border border-yellow-500/30 rounded-md hover:bg-yellow-500/20 disabled:opacity-50 disabled:cursor-wait">
                                <DownloadIcon className="w-4 h-4" />
                                Compartir
                            </button>
                        </div>
